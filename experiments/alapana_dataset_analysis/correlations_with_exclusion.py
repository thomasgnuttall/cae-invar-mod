...
results_df_path = os.path.join(results_dir, 'results_high_energy.csv')
sig_results_df_path = os.path.join(results_dir, 'results_significant_high_energy.csv')
...
audio_distance = audio_distance[audio_distance['length_mismatch']!=True]
...
def hdf(i1, i2):
    hand1 = all_groups[all_groups['index']==i1]['handedness'].values[0]
    hand2 = all_groups[all_groups['index']==i2]['handedness'].values[0]
    if hand1 != hand2:
        return 'Both'
    else:
        return hand1

audio_distance['hand'] = audio_distance.apply(lambda y: hdf(y.index1, y.index2), axis=1)

## Get bad indices
energy_thresh = 0.005

bad_indices = all_groups[all_groups['HandEnergy']<energy_thresh]['index'].values

audio_distance['no_energy'] = (audio_distance['index1'].isin(bad_indices)) | (audio_distance['index2'].isin(bad_indices))

audio_distance_cut = audio_distance[audio_distance['no_energy']==False]

prop = len(audio_distance_cut)/len(audio_distance)

print(f"{round(prop*100,2)}% of observations used")
...
for i, row in tqdm.tqdm(list(significant_results.iterrows())):
    ...
    out_path = os.path.join(results_dir, 'plots_hand', x, y, level, (level if level == 'all' else str(level_value)) + '.png')
    create_if_not_exists(out_path)

    if level != 'all':
        cols = [x for x in audio_distance.columns if level in x]
        data = audio_distance[(audio_distance[cols[0]]==level_value) & (audio_distance[cols[1]]==level_value)]
    else:
        data = audio_distance


    pl = sns.scatterplot(data, x=x, y=y, s=5, c=data['hand'], cmap=matplotlib.colors.ListedColormap(['black', 'red', 'blue']))
    pl.set_title(title)
    fig = pl.get_figure()
